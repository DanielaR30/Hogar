...
function listar()
{
    ...
    tabla=$('#tbllistado').dataTable(
        ...
        "ajax":
                {
                    url: '../ajax/tipogasto.php?op=listar',
                    type: "get",
                    dataType: "json",
...
function guardaryeditar(e){
	...
	$.ajax({
		url: "../ajax/tipogasto.php?op=guardaryeditar",
	    type: "POST",
	    data: formData,
	    contentType: false,
	    processData: false,
...
function mostrar(idtipogasto){
    $.post("../ajax/tipogasto.php?op=mostrar",{idtipogasto :  idtipogasto}, function(data, status)
    {
        data = JSON.parse(data);
        mostrarform(true);
        $("#nombre").val(data.nombre);
        $("#idtipogasto").val(data.idtipogasto);
    })
}
//Funcion para desactivar registros
function desactivar(idtipogasto)
{
    //El parametro result recibe true o false dependiendo del clic del usuario al mensaje de confirmacion
    bootbox.confirm("¿Esta seguro de desactivar la tipogasto?", function(result){
        //Si result es verdadero, se desactivara la categoria de lo contrario no 
        if(result)
        {
            //e es el mensaje que recibe la url 
            $.post("../ajax/tipogasto.php?op=desactivar",{idtipogasto : idtipogasto}, function(e){
                bootbox.alert(e);
...
function activar(idtipogasto)
{
    bootbox.confirm("¿Esta seguro de activar la tipogasto?", function(result){
        if(result)
        {
            $.post("../ajax/tipogasto.php?op=activar", {idtipogasto : idtipogasto}, function(e){
                bootbox.alert(e);
                tabla.ajax.reload();
            });
        }
