...
function listar()
{
    ...
    tabla=$('#tbllistado').dataTable(
        ...
        "ajax":
                {
                    url: '../ajax/veterinaria.php?op=listar',
                    type: "get",
                    dataType: "json",
                    error: function(e){
...
function guardaryeditar(e){
	...
	$.ajax({
		url: "../ajax/veterinaria.php?op=guardaryeditar",
	    type: "POST",
	    data: formData,
	    contentType: false,
	    processData: false,
...
function mostrar(idveterinaria){
    $.post("../ajax/veterinaria.php?op=mostrar",{idveterinaria :  idveterinaria}, function(data, status)
    {
        data = JSON.parse(data);
        mostrarform(true);
        $("#nombre").val(data.nombre);
        $("#funcionario").val(data.funcionario);
        $("#idveterinaria").val(data.idveterinaria);
    })
}
//Funcion para desactivar registros
function desactivar(idveterinaria)
{
    //El parametro result recibe true o false dependiendo del clic del usuario al mensaje de confirmacion
    bootbox.confirm("¿Esta seguro de desactivar la veterinaria?", function(result){
        //Si result es verdadero, se desactivara la categoria de lo contrario no 
        if(result)
        {
            //e es el mensaje que recibe la url 
            $.post("../ajax/veterinaria.php?op=desactivar",{idveterinaria : idveterinaria}, function(e){
                bootbox.alert(e);
...
function activar(idveterinaria)
{
    bootbox.confirm("¿Esta seguro de activar la veterinaria?", function(result){
        if(result)
        {
            $.post("../ajax/veterinaria.php?op=activar", {idveterinaria : idveterinaria}, function(e){
                bootbox.alert(e);
                tabla.ajax.reload();
            });
        }
